...
public class AdminShopCommand {
	...
	private static int setName(CommandContext<CommandSourceStack> ctx) {
		...
		CompoundTag tag = itemInHand.getOrDefault(DataComponents.BLOCK_ENTITY_DATA, CustomData.EMPTY).copyTag();
		if (tag.contains("adminShopName") && tag.getString("adminShopName").equals(name)) return 1;
	...
	private static int resetName(CommandContext<CommandSourceStack> ctx) {
		...
		CompoundTag tag = itemInHand.getOrDefault(DataComponents.BLOCK_ENTITY_DATA, CustomData.EMPTY).copyTag();
		if (!tag.contains("adminShopName")) return 1;
